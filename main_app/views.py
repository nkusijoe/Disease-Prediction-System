...
def checkdisease(request):
  ...
  disease_to_symptoms = {
    'itching': ['skin_rash', 'nodal_skin_eruptions', 'scurring', 'skin_peeling', 'blister', 'red_sore_around_nose', 'yellow_crust_ooze', 'dischromic_patches', 'pus_filled_pimples', 'blackheads'],
    'silver_like_dusting': ['small_dents_in_nails', 'inflammatory_nails'],
    'continuous_sneezing': ['cough', 'phlegm', 'throat_irritation', 'sinus_pressure', 'runny_nose', 'congestion', 'mucoid_sputum', 'rusty_sputum'],
    'high_fever': ['shivering', 'chills', 'breathlessness', 'sweating', 'malaise', 'watering_from_eyes'],
    'stomach_pain': ['acidity', 'ulcers_on_tongue', 'vomiting', 'indigestion', 'loss_of_appetite', 'diarrhoea', 'constipation', 'abdominal_pain', 'belly_pain', 'distention_of_abdomen', 'stomach_bleeding'],
    'burning_micturition': ['spotting_urination', 'dark_urine', 'yellow_urine', 'bladder_discomfort', 'foul_smell_of urine', 'continuous_feel_of_urine', 'passage_of_gases'],
    'fatigue': ['weight_gain', 'cold_hands_and_feets', 'mood_swings', 'weight_loss', 'lethargy', 'irregular_sugar_level', 'excessive_hunger', 'polyuria'],
    'enlarged_thyroid': ['brittle_nails', 'puffy_face_and_eyes'],
    'anxiety': ['restlessness', 'dizziness', 'spinning_movements', 'loss_of_balance', 'unsteadiness', 'headache', 'lack_of_concentration', 'visual_disturbances', 'altered_sensorium', 'depression', 'irritability'],
    'muscle_pain': ['joint_pain', 'muscle_wasting', 'muscle_weakness', 'cramps', 'knee_pain', 'hip_joint_pain', 'movement_stiffness'],
    'receiving_blood_transfusion': ['receiving_unsterile_injections', 'blood_in_sputum', 'prominent_veins_on_calf', 'palpitations'],
    'patches_in_throat': ['red_spots_over_body', 'abnormal_menstruation', 'family_history'],

    }
    
  def group_symptoms_by_disease(symptoms_sorted, disease_to_symptoms_mapping):
        grouped_data = {}

        for disease, symptoms in disease_to_symptoms_mapping.items():
            for symptom in symptoms:
                if symptom in symptoms_sorted:
                    if disease in grouped_data:
                        grouped_data[disease].append(symptom)
                    else:
                        grouped_data[disease] = [symptom]

        return grouped_data

  grouped_symptoms = group_symptoms_by_disease(alphabaticsymptomslist, disease_to_symptoms)  

    # # Handle GET request
    # if request.method == 'GET':
    #     return render(request, 'patient/checkdisease/checkdisease.html', {"list2": alphabaticsymptomslist, "grouped_data": grouped_symptoms})

    # # Handle POST request
    # elif request.method == 'POST':
    #     # ... [your POST request handling here] ...

    # # For demonstration purpose: Returning the grouped symptoms for both GET and POST. Modify as per requirement.
    # return JsonResponse(grouped_symptoms, safe=False)
...
def download_content_as_pdf(request):
    if request.method == 'POST':
        data = json.loads(request.body)
        html_content = data.get('content', '')
        desired_size = data.get('size', '')

        # Create a buffer for the PDF
        buffer = BytesIO()

        # Define the A4 size and then calculate quarter size
        a4_width_mm = 210
        a4_height_mm = 297
        quarter_a4_width = a4_width_mm / 2
        quarter_a4_height = a4_height_mm / 2
        
        # If quarter size is requested, set the size accordingly
        if desired_size == 'quarter_a4':
            pdf_size = (quarter_a4_width, quarter_a4_height)
        else:
            pdf_size = (a4_width_mm, a4_height_mm)

        # Convert HTML to PDF
        pisa.CreatePDF(html_content, dest=buffer, defaultPageSize=pdf_size)
...
def post(request):
    if request.method == "POST":
        ...
        if msg != '':            
            ...
            doctor_phone_number = consultation_obj.doctor.mobile_no
            client.messages.create(
                to=doctor_phone_number,
                from_=settings.TWILIO_PHONE_NUMBER,
                body="You have a new message from " + request.user.username + ": EMERGENCY HELP IN NEED OF YOUR PATIENT!!!!!" + msg
            )
            print("msg saved"+ msg )
